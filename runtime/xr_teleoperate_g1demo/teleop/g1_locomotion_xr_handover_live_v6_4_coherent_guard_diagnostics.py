#!/usr/bin/env python3
"""Live G1 Regular-mode locomotion <-> XR arm handover (V6.4 diagnostics).
...
- Wrist jump rejection is bimanually coherent: if either wrist is rejected,
  BOTH wrist targets hold the last coherent pair until a stable pair is
  reacquired. One stale wrist is never mixed with one newly accepted wrist.
...
def update_guarded_pair(
    ...
    V6.4 accepts or holds BOTH wrists together. If either wrist jumps relative
    to the last coherent pair, the previous pair is held. A discontinuous new
    pair is accepted only after both candidate wrists remain mutually stable
    for ``reacquire_frames`` consecutive samples.
    """
...
def main() -> int:
    ...
    try:
        ...
        LOG.info("G1_LOCOMOTION_XR_HANDOVER_LIVE_V6_4_COHERENT_GUARD_DIAGNOSTICS")
        LOG.warning(
