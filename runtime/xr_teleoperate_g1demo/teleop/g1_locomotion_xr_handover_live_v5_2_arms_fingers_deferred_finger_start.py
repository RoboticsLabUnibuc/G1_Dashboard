...
class FingerController:
    ...
    def start_worker(self) -> None:
        """Load retargeting and start 60 Hz control exactly once."""
        ...
        with self._start_lock:
            ...
            LOG.info(
                "Valid Quest XR frame received. Loading Inspire retargeting "
                "and starting the 60 Hz finger worker."
            )
            ...
            try:
                ...
                self._worker_error = (
                    f"deferred start {type(exc).__name__}: {exc}"
                )
                self._update_status()
                raise
...
def main() -> int:
    ...
    try:
        ...
        LOG.info("G1_LOCOMOTION_XR_HANDOVER_LIVE_V5_2_ARMS_FINGERS_DEFERRED_FINGER_START")
        LOG.warning(
        ...
        while not FORCE_STOP.is_set():
            ...
            if xr_valid and not finger_ctrl.worker_started():
                try:
                    finger_ctrl.start_worker()
                except Exception as exc:
                    LOG.exception(
                        "Could not start deferred finger worker: %s",
                        exc,
                    )
