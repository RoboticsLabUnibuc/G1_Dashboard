#!/usr/bin/env python3
"""Quest hand-tracking -> Inspire DFX finger-only test V5 for Unitree G1 PC2.
...
def direct_child_pids(pid: int) -> list[int]:
    """Return direct Linux child PIDs using procfs."""
...
def descendant_pids(root_pid: int) -> list[int]:
    """Return all current descendants, parents before children."""
    ordered: list[int] = []
    seen: set[int] = set()
    ...
    while stack:
        ...
        children = direct_child_pids(pid)
...
def kill_all_descendants() -> None:
    """Kill all subprocess descendants after safe hand opening.

    A descendant can retain the stdout pipe inherited from this process,
    causing ``tee`` and the launching shell to wait even after the main
    ...
    for attempt in range(1, 4):
        ...
        # Kill deepest descendants first, then their parents.
        for pid in reversed(descendants):
            try:
                ...
                LOG.warning("Could not kill descendant pid=%d: %s", pid, exc)
...
def main() -> int:
    ...
    try:
        ...
        # Capture and terminate every descendant, not only TeleVuer's direct
        # child. A grandchild retaining stdout/stderr keeps the shell pipeline
        # open even after os._exit() in the parent.
        kill_all_descendants()
        LOG.info("Shutdown cleanup complete; calling os._exit().")
