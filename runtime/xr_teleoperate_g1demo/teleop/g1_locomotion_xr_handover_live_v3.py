...
def main() -> int:
    ...
    try:
        ...
        LOG.info("G1_LOCOMOTION_XR_HANDOVER_LIVE_V3")
        LOG.warning(
            "LIVE ARM COMMANDS ENABLED: publishing rt/arm_sdk."
        )
        LOG.info(
            "V3 limiter active: user arm commands accumulate from the "
        ...
        while not FORCE_STOP.is_set():
            ...
            if SNAPSHOT_REQUESTED:
                ...
                if (
                    ...
                    xr_wrist_sep_text = f"{xr_wrist_sep:.3f} m"
                ...
                LOG.info(
                    ...
                    "wrist-separation[xr/measured/target/published]="
                    "%s/%s/%s/%s current_q=%s",
                    state.name,
                    arm_ctrl.get_weight(),
