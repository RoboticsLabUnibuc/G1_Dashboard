...
class OrbitControlsCompat(SceneElement):
    """Compatibility schema for clients that support the OrbitControls tag.

    vuer==0.0.60 does not export OrbitControls from vuer.schemas, but its
    schema serializer can still send an arbitrary supported component tag.
    """
    tag = "OrbitControls"
...
def _extract_camera_matrix(value: object) -> tuple[np.ndarray | None, str]:
    ...
    for candidate, path in candidates:
        ...
        if matrix.size == 16 and np.isfinite(matrix).all():
            return matrix.reshape(4, 4), path
    return None, "<no supported 4x4 matrix field>"


async def patched_on_cam_move(self, event, session, fps=60):
    count = int(getattr(self, "_camera_stream_probe_count", 0)) + 1
    self._camera_stream_probe_count = count

    value = getattr(event, "value", None)
    matrix, source = _extract_camera_matrix(value)

    if count == 1:
        keys = list(value.keys()) if isinstance(value, dict) else None
        print(
            "[CAMERA_MOVE probe] first event received; "
            f"event.key={getattr(event, 'key', None)!r}, value_keys={keys!r}",
            flush=True,
        )
    ...
    if matrix is None:
        if count <= 5 or count % 120 == 0:
            print(
                f"[CAMERA_MOVE probe] event #{count}: no supported matrix; "
                f"value={value!r}",
                flush=True,
            )
        return
    ...
    with self.head_pose_shared.get_lock():
        # TeleVuer's getter reshapes this shared array with order="F".
        # Preserve the original handler's flat-list convention.
        self.head_pose_shared[:] = matrix.reshape(-1).tolist()

    if count <= 3 or count % 120 == 0:
        print(
            f"[CAMERA_MOVE probe] event #{count}: updated from {source}; "
            f"raw translation={np.array2string(matrix[:3, 3], precision=3)}",
            flush=True,
        )


async def patched_main_pass_through(self, session):
    tracking = (
        Hands(
            stream=True,
            key="hands",
    ...
    session.upsert(tracking, to="bgChildren")
    session.upsert(
        OrbitControlsCompat(
            key="OrbitControls",
            stream=True,
    ...
    print(
        "[Probe] Inserted compatibility OrbitControls(tag='OrbitControls', "
        "stream=True) to request main-camera/headset updates.",
        flush=True,
    )
...
# Install the temporary patches before TeleVuerWrapper creates TeleVuer.
...
def main() -> int:
    ...
    LOG.info("[Probe] EXPLICIT_HEAD_STREAM_PROBE_V5 -- read-only temporary scene patch")
