...
- Lost XR/hand tracking freezes the current arm and finger commands without
  entering safety-fault mode. Control resumes only after the operator returns
  both wrists and fingers near the frozen pose for several stable frames.
...
def main() -> int:
    ...
    try:
        ...
        LOG.info("G1_LOCOMOTION_XR_HANDOVER_LIVE_V6_2_CAMERA_PORT_CLEANUP_FIX")
        LOG.warning(
        ...
        LOG.info(
            "Tracking-loss hold resumes at wrist error<=%.3f m / %.1f deg "
            "for %d frames; fingers must be within %.2f normalized units.",
            ...
            if active_control_state:
                ...
                if fault_reason is not None:
                    ...
                    LOG.warning(
                        ...
                        "and fingers near the frozen pose to resume.",
                        tracking_hold_reason,
                    )
                else:
            ...
            if state == State.XR_TRACKING_HOLD:
                ...
                if hold_fault_reason is not None:
                    ...
                    if (
                        ...
                        if wrists_ready and finger_reacquire_ready:
                        ...
                        if (
                            ...
                            if resume_state == State.ARM_RAMP_UP:
                                ...
                                state_started = now
                                LOG.warning(
                                    "XR tracking reacquired at the frozen "
                                    "pose. Resuming ownership ramp from %.3f.",
                                    current_weight,
                                )
                            else:
                                state = State.XR_ACTIVE
                                ...
                                LOG.warning(
                                    "XR tracking reacquired at the frozen "
                                    "pose. Resuming live teleoperation."
                                )
                    else:
                        tracking_hold_stable_count = 0
                else:
