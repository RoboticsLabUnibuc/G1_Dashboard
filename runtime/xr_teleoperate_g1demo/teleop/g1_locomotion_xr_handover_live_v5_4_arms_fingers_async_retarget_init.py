...
class FingerController:
    ...
    def __init__(
        ...
        *,
        ...
        self._retargeting: Optional[object] = None
    ...
    def request_worker_start(self) -> None:
        """Request retargeting initialization without blocking the arm loop."""
        ...
        with self._start_lock:
            ...
            with self._lock:
                ...
                self._status["tracking_reason"] = (
                    "loading Inspire retargeting in background"
                )

            LOG.info(
                "Valid Quest XR frame received. Starting asynchronous "
                "Inspire retargeting initialization."
            )

            self._init_thread = threading.Thread(
                target=self._initialize_worker,
                name="g1-inspire-retarget-initializer",
                daemon=True,
            )
            self._init_thread.start()

    def _initialize_worker(self) -> None:
        """Load dex-retargeting, then start the 60 Hz publisher worker."""
        try:
            # The controller is launched from teleop/, matching the proven
            # finger-only program. Do not call os.chdir() from this background
            # thread because process working-directory changes are global.
            ...
            self._thread.start()
            LOG.info(
                "Inspire retargeting initialized. "
                "The 60 Hz finger worker is running."
            )
        except Exception as exc:
            self._worker_error = (
                f"asynchronous start {type(exc).__name__}: {exc}"
            )
            self._update_status(
                worker_initializing=False,
                tracking_reason=self._worker_error,
            )
            LOG.exception("Asynchronous finger initialization failed")
    ...
    def close(self) -> None:
        ...
        if init_thread is not None and init_thread.is_alive():
            init_thread.join(timeout=0.25)
            if init_thread.is_alive():
                LOG.warning(
                    "Retargeting initializer is still active; "
                    "it is a daemon and will not delay safe process exit."
                )
    ...
    def _run(self) -> None:
        if self._retargeting is None:
            self._worker_error = "retargeting was not initialized"
            self._update_status()
            return
...
def main() -> int:
    ...
    try:
        ...
        LOG.info("G1_LOCOMOTION_XR_HANDOVER_LIVE_V5_4_ARMS_FINGERS_ASYNC_RETARGET_INIT")
        LOG.warning(
