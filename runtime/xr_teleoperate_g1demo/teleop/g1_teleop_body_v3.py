...
# PAUSE_RECENTER_V1
# WRIST_BODY_FRAME_V3_HEAD_POSITION
...
_OPERATOR_FRAME_MIN_FORWARD_REACH_M = 0.12
_OPERATOR_FRAME_MAX_HEAD_ALIGNMENT_DEG = 70.0
...
def wrist_position_world_relative(
    ...
    TeleVuer is configured with arm_reference_mode="head_position" in this
    version. It subtracts the headset position but deliberately does not rotate
    the wrist into the live headset-yaw frame. Only the fixed waist offset has
    to be removed here.

    current_head_yaw is accepted for API compatibility and is used separately
    by build_operator_body_frame only to resolve the 180-degree sign ambiguity.
    """
...
def build_operator_body_frame(
    ...
    """
    Estimate an operator body-yaw frame from a standard two-hand pose.
    ...
    The wrist line supplies the lateral axis. That line alone has a 180-degree
    ambiguity, so headset yaw is used only to choose the forward-facing branch.
    It is not used for continuous wrist control after calibration.
    """
    ...
    left_axis = left_world - right_world
    left_axis[2] = 0.0
    wrist_separation = float(np.linalg.norm(left_axis))

    if (
        not np.isfinite(wrist_separation)
        or wrist_separation
        < _OPERATOR_FRAME_MIN_WRIST_SEPARATION_M
    ):
        raise ValueError(
    ...
    left_axis /= wrist_separation

    z_axis = np.array([0.0, 0.0, 1.0], dtype=float)

    # Robot convention: x forward, y left, z up, therefore x = y cross z.
    forward_axis = np.cross(left_axis, z_axis)
    ...
    # Resolve the unavoidable 180-degree ambiguity using headset yaw only as
    # a sign reference. Flipping both x and y preserves a right-handed frame.
    ...
    raw_alignment = float(np.dot(forward_axis, head_forward))
    ...
    head_alignment_deg = float(
        np.degrees(
            np.arccos(np.clip(raw_alignment, -1.0, 1.0))
        )
    )

    if head_alignment_deg > _OPERATOR_FRAME_MAX_HEAD_ALIGNMENT_DEG:
        raise ValueError(
            "look roughly forward during capture; headset and wrist-derived "
            f"forward differ by {head_alignment_deg:.1f} deg, maximum is "
            f"{_OPERATOR_FRAME_MAX_HEAD_ALIGNMENT_DEG:.0f} deg"
        )
    ...
    forward_reach = float(np.dot(midpoint_horizontal, forward_axis))
    lateral_offset = float(np.dot(midpoint_horizontal, left_axis))

    if (
        not np.isfinite(forward_reach)
        or forward_reach < _OPERATOR_FRAME_MIN_FORWARD_REACH_M
    ):
        measured_text = (
            "non-finite"
            if not np.isfinite(forward_reach)
            else f"{forward_reach:.3f} m"
        )
        raise ValueError(
            "hold both hands clearly in front of the body; "
            f"measured forward reach is {measured_text}, minimum is "
            f"{_OPERATOR_FRAME_MIN_FORWARD_REACH_M:.2f} m"
        )

    operator_frame = np.column_stack(
        [forward_axis, left_axis, z_axis]
    )
...
def express_pose_in_operator_frame(
    ...
    """
    Convert a pose from TeleVuer's live head-yaw frame into the
    wrist-derived operator body frame captured during calibration.
    """
    pose = np.asarray(pose, dtype=float)
    corrected = pose.copy()

    correction = (
        operator_frame.T @ current_head_yaw
    )
...
# SAVED_TRANSLATION_PROFILE_V3_BODY_FRAME
TRANSLATION_PROFILE_VERSION = 3
TRANSLATION_PROFILE_OPERATOR_FRAME = "wrist_body_v3_head_position"
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger_mp.info(
            ...
            "standard forward neutral pose: wrists separated left/right "
            "and both hands clearly in front of the body."
        )
        logger_mp.info(
        ...
        # GUARDED_TRACKING_V1 + WRIST_BODY_FRAME_V3 state.
        ...
        while not STOP:
            ...
            if not arm_calibrated:
                ...
                if calibration_ready_frames == 1:
                    logger_mp.info(
                        "[Arm calibration] Hold the standard forward "
                        "neutral pose still: wrists separated left/right, "
                        "both hands in front, and look roughly straight ahead..."
                    )
            ...
            if RECENTER_TOGGLE:
                ...
                if not arm_calibrated:
                    ...
                    logger_mp.info(
                        ...
                        "the standard forward neutral pose: wrists separated "
                        "left/right and both hands clearly in front of the "
                        "body. Hold still, then press [c] again."
                    )
                    ...
                    logger_mp.info(
                        "[Recenter] RESUME requested. Hold the standard "
                        "forward neutral pose still while the wrist-derived "
                        "body frame is captured."
                    )
