...
Keys
----
r : initial engage after matching your arms to the robot pose
c : clutch: pause; press again to register translation and resume
...
def on_press(key: str) -> None:
    global START, STOP, SNAPSHOT, CLUTCH_TOGGLE

    if key == "r":
        if START:
            LOG.warning("[r] initial engage was already requested.")
        else:
            START = True
            LOG.info("[r] initial engage requested.")
    elif key == "c":
        CLUTCH_TOGGLE = True
        LOG.info("[c] clutch toggle requested.")
    elif key == "p":
        SNAPSHOT = True
    elif key == "q":
        START = False
        ...
        LOG.warning("Key %r has no action. Use r, c, p, or q.", key)
...
def compute_shared_translation_bias(
    robot_left: np.ndarray,
    robot_right: np.ndarray,
    live_left: np.ndarray,
    live_right: np.ndarray,
    mode: str,
) -> np.ndarray:
    """Align the live two-wrist centre to the robot two-wrist centre.

    The same translation is applied to both wrists. This preserves hand
    separation, wrist orientations, motion directions, and the live head-yaw
    frame. No rotation, scaling, per-arm offset, or relative neutral transform
    is introduced.
    """
    robot_centre = 0.5 * (
        robot_left[:3, 3] + robot_right[:3, 3]
    )
    live_centre = 0.5 * (
        live_left[:3, 3] + live_right[:3, 3]
    )
    bias = robot_centre - live_centre
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=0.12,
        help=(
            "Maximum residual position mismatch per wrist after the shared "
            "engage translation is applied."
        ),
    )
    parser.add_argument(
        "--engage-registration",
        ...
        default="xyz",
        help=(
            "Common translation captured at initial engage and clutch resume. "
            "'xyz' aligns the two-wrist centre, 'z' corrects height only, and "
            "'none' preserves strict absolute TeleVuer positions."
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=0.30,
        help="Maximum permitted magnitude of the common engage translation.",
    )
    parser.add_argument(
        "--stable-start-frames",
        type=int,
    ...
    parser.add_argument(
        ...
        default=3,
        help=(
            "Consecutive invalid XR frames before automatic clutch pause. "
            "Commands are held during the grace frames."
        ),
    )
    parser.add_argument(
        "--status-hz",
...
def main() -> int:
    ...
    if args.max_registration_shift_m <= 0:
        raise ValueError("--max-registration-shift-m must be positive")
    if args.stable_start_frames < 1:
        raise ValueError("--stable-start-frames must be at least 1")
    if args.tracking_loss_frames < 1:
        raise ValueError("--tracking-loss-frames must be at least 1")
    ...
    try:
        ...
        LOG.info("DIRECT_HEAD_YAW_ARM_ONLY_TRANSLATION_CLUTCH_V3")
        LOG.info(
        ...
        LOG.info(
            ...
            "[c] pauses/resumes through alignment; [p] snapshot; [q] exit."
        )
        LOG.info(
            "Engage/resume registration=%s. It applies one common translation "
            "to both wrists; no rotation, scaling, per-arm offset, or body "
            "frame is calculated.",
            args.engage_registration,
        )
        ...
        def hold_last_command(
            current_q: np.ndarray,
            current_dq: np.ndarray,
        ) -> None:
            if last_left_command is None or last_right_command is None:
                return
            hold_q, hold_tauff = arm_ik.solve_ik(
                last_left_command,
                last_right_command,
                current_q,
                current_dq,
        ...
        while not STOP:
            ...
            # Process one clutch request per loop.
            if CLUTCH_TOGGLE:
                CLUTCH_TOGGLE = False
                ...
                if state == "RUNNING":
                    ...
                    consecutive_invalid_frames = 0
                    LOG.info(
                        "PAUSED by [c]. Robot holds its current arm target. "
                        "Move/reposition freely, then match the robot pose and "
                        "press [c] again."
                    )
                elif state == "PAUSED":
                    state = "RESUME_ALIGN"
                    ...
                    last_align_log = 0.0
                    LOG.info(
                        "Resume requested. Match both hands to the robot's "
                        "current arm pose; a common translation will be "
                        "registered before control resumes."
                    )
                elif state == "RESUME_ALIGN":
                    state = "PAUSED"
                    stable_align_count = 0
                    align_bias_sum[:] = 0.0
                    align_bias_samples = 0
                    LOG.info(
                        "Resume request cancelled; remaining paused."
                    )
                else:
                    LOG.warning(
                        "Initial alignment is still pending. Use [r] once, "
                        "match the robot pose, and wait for ENGAGED."
                    )

            live_left: Optional[np.ndarray] = None
            live_right: Optional[np.ndarray] = None
            ...
            if state == "ALIGNING":
                alignment_left: Optional[np.ndarray] = None
                alignment_right: Optional[np.ndarray] = None

                if not frame_valid or live_left is None or live_right is None:
                    stable_align_count = 0
                    align_bias_sum[:] = 0.0
                    align_bias_samples = 0
                else:
                    ...
                    candidate_shift = float(np.linalg.norm(candidate_bias))
                    alignment_left = apply_translation_bias(
                        live_left,
                        candidate_bias,
                    )
                    alignment_right = apply_translation_bias(
                        live_right,
                        candidate_bias,
                    )

                    left_align_error = float(
                        np.linalg.norm(
                            alignment_left[:3, 3]
                            - current_left[:3, 3]
                        )
                    )
                    right_align_error = float(
                        np.linalg.norm(
                            alignment_right[:3, 3]
                            - current_right[:3, 3]
                        )
                    ...
                    now = time.monotonic()
                    alignment_ok = (
                        max_align_error <= args.start_align_m
                        and candidate_shift
                        <= args.max_registration_shift_m
                    )

                    if alignment_ok:
                        stable_align_count += 1
                    ...
                    if now - last_align_log >= 1.0:
                        LOG.info(
                            "Initial registration: bias=%s (|d|=%.3f m), "
                            "residual L/R=%.3f/%.3f m "
                    ...
                    if (
                        stable_align_count >= args.stable_start_frames
                        ...
                        last_good_left = registered_left.copy()
                        last_good_right = registered_right.copy()
                        guarded_left = registered_left.copy()
                        guarded_right = registered_right.copy()
                        left_pending = None
                        right_pending = None
                        left_pending_count = 0
                        right_pending_count = 0

                        # Start the limiter at the current robot FK. The common
                        # translation makes the live two-wrist centre coincide
                        # with the current robot two-wrist centre, preventing
                        # the systematic engage drop/shift.
                        ...
                        LOG.info(
                            "ENGAGED: shared translation bias=%s "
                            "(|d|=%.3f m).",
                            format_vector(translation_bias),
                            final_shift,
                        )
                        LOG.info(
                            "Direct live head_yaw motion and wrist rotations "
                            "remain unchanged; only the common workspace origin "
                            "was aligned."
                        )
            ...
            if state == "RUNNING":
                if (
                    not frame_valid
                    or registered_left is None
                    or registered_right is None
                ):
                    consecutive_invalid_frames += 1
                    hold_last_command(current_q, current_dq)

                    if (
                        consecutive_invalid_frames
                        >= args.tracking_loss_frames
                    ):
                        state = "PAUSED"
                        pause_reason = "tracking loss"
                        stable_align_count = 0
                        LOG.warning(
                            "AUTO-PAUSED after %d invalid tracking frames. "
                            "The robot is holding its last arm target. Restore "
                            "tracking, match the robot pose, then press [c].",
                            consecutive_invalid_frames,
                        )
                ...
                if periodic_status:
                    ...
                    LOG.info(
                        "RUN head_yaw=%+.1f deg | target error "
                        "L/R=%.3f/%.3f m | guard=%s",
                        robot_yaw_deg(
                            np.asarray(
                                tele_data.head_pose,
            ...
            elif state in ("PAUSED", "RESUME_ALIGN"):
                # Keep publishing the held Cartesian target so the arm
                # controller remains active while the operator reclutches.
                hold_last_command(current_q, current_dq)

                if state == "RESUME_ALIGN":
                    if not frame_valid or live_left is None or live_right is None:
                        stable_align_count = 0
                        align_bias_sum[:] = 0.0
                        align_bias_samples = 0
                        ...
                        if now - last_align_log >= 1.0:
                            LOG.info(
                                "Resume registration: bias=%s "
                                "(|d|=%.3f m), residual L/R=%.3f/%.3f m "
                                "(limit %.3f m), stable=%d/%d.",
                                format_vector(candidate_bias),
                                candidate_shift,
                                left_align_error,
                        ...
                        if (
                            ...
                            LOG.info(
                                "RESUMED: shared translation bias=%s "
                                "(|d|=%.3f m).",
                                format_vector(translation_bias),
                                final_shift,
                            )
                            LOG.info(
                                "No yaw/rotation recenter or relative neutral "
                                "offset was applied."
                            )
                ...
                if periodic_status:
                    ...
                    LOG.info(
                        "%s | reason=%s | head_yaw=%s deg | "
                        "registration=%s | press [c] %s",
                        state,
                        pause_reason,
                        head_yaw_text,
                        format_vector(translation_bias),
