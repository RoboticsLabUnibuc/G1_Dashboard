...
if __name__ == '__main__':
    ...
    try:
        ...
        # FULL_RANGE_SPEED_LIMIT_V1
        # Full workspace mapping, with fixed joint-space speed limiting.
        arm_ctrl.arm_velocity_limit = 0.5
        logger_mp.info(
            "Full-range arm mapping enabled; "
            "joint velocity limit = 0.5 rad/s."
        )
        head_img = None
        left_wrist_img = None
        ...
        arm_position_gain = 1.0  # full operator-to-robot translation range
        while not STOP:
            ...
            # Ignore missing or temporarily lost XR tracking frames.
            if not tele_data.motion_data_ready:
                time.sleep(0.01)
                continue

            # Let WebXR tracking settle for about one second after [r].
            if not arm_calibrated:
                calibration_ready_frames += 1
                if calibration_ready_frames == 1:
                    logger_mp.info("[Arm calibration] Hold both hands still...")
                if calibration_ready_frames < 30:
                    time.sleep(0.01)
                    continue

                xr_left_start_pose = tele_data.left_wrist_pose.copy()
                xr_right_start_pose = tele_data.right_wrist_pose.copy()
                ...
                arm_calibrated = True
                logger_mp.info("[Arm calibration] Complete.")
                logger_mp.info("[Arm calibration] The current operator pose is now neutral.")

            left_wrist_target = tele_data.left_wrist_pose.copy()
            right_wrist_target = tele_data.right_wrist_pose.copy()

            # Translation is relative to the Quest pose captured at calibration.
            left_wrist_target[:3, 3] = (
                robot_left_start_pose[:3, 3]
                + arm_position_gain
                * (tele_data.left_wrist_pose[:3, 3]
                   - xr_left_start_pose[:3, 3])
            )

            right_wrist_target[:3, 3] = (
                robot_right_start_pose[:3, 3]
                + arm_position_gain
                * (tele_data.right_wrist_pose[:3, 3]
                   - xr_right_start_pose[:3, 3])
            )

            # Apply wrist rotation relative to the calibration orientation.
            left_wrist_target[:3, :3] = (
                robot_left_start_pose[:3, :3]
