...
def wrist_position_world_relative(
    ...
    """
    Reconstruct the wrist position relative to the headset in the
    Quest world horizontal frame.

    TeleVuer has already expressed the wrist in the live head-yaw frame
    and added the fixed waist offset, so undo those two operations here.
    """
...
def build_operator_body_frame(
    ...
    Required pose:
      - both wrists visible;
      - wrists separated left/right;
      - both hands held in front of the body.
    ...
    forward_norm = float(
        np.linalg.norm(forward_axis)
    )

    if not np.isfinite(forward_norm) or forward_norm < 1e-6:
        raise ValueError(
            "could not construct a horizontal forward axis"
        )
    ...
    # A horizontal wrist line determines body yaw only up to 180 degrees.
    # Resolve that ambiguity by requiring the forward axis to point toward
    # the midpoint of the two wrists. Flipping both horizontal axes keeps
    # the frame right-handed while preserving the measured wrist line.
    if np.isfinite(forward_reach) and forward_reach < 0.0:
        forward_axis *= -1.0
        left_axis *= -1.0
        forward_reach *= -1.0
    ...
    if (
        ...
        or forward_reach
        < _OPERATOR_FRAME_MIN_FORWARD_REACH_M
    ):
        measured_text = (
            "non-finite"
        ...
        raise ValueError(
            ...
            f"measured forward reach is {measured_text}, "
            f"minimum is "
    ...
    if (
        ...
        raise ValueError(
            "operator body frame is invalid"
        )
...
def build_horizontal_translation_map(
    ...
    if not np.isfinite(determinant) or abs(determinant) < 0.50:
        raise ValueError(
            "Registration gestures are too close to the same direction."
        )
...
# SAVED_TRANSLATION_PROFILE_V2_BODY_FRAME
TRANSLATION_PROFILE_VERSION = 2
TRANSLATION_PROFILE_OPERATOR_FRAME = "wrist_body_v2"
    ...
    try:
        ...
        if (
            ...
            logger_mp.warning(
                ...
                "A new wrist-body-frame-v2 profile must be learned once."
            )
            return None
...
if __name__ == '__main__':
    ...
    try:
        ...
        # GUARDED_TRACKING_V1 + WRIST_BODY_FRAME_V1 state.
            ...
            if not arm_calibrated:
                ...
                if calibration_ready_frames == 1:
                    logger_mp.info(
                        ...
                        "and both hands in front of the body..."
                    )
