...
def configure_logging() -> logging.Logger:
    logger = logging.getLogger("g1_locomotion_xr_live")
...
class SafeArmSdkController:
    ...
    controller starts at ownership weight zero and seeds its arm target from
    the measured robot pose before any non-zero weight is permitted.
    """
    ...
    def _publish_loop(self) -> None:
        while not self._stop.is_set():
            ...
            lowstate, age, _ = self._cache.snapshot()
            current_q: Optional[np.ndarray] = None

            if lowstate is not None and age <= 0.25:
                try:
                    current_q = np.asarray(
                        [
                            float(lowstate.motor_state[i].q)
            ...
            if (
                current_q is None
                or current_q.shape != (14,)
                or not np.isfinite(current_q).all()
            ):
                current_q = self._last_sent_q.copy()
...
def main() -> int:
    ...
    try:
        ...
        LOG.info("G1_LOCOMOTION_XR_HANDOVER_LIVE_V1_1")
        LOG.warning(
            "LIVE ARM COMMANDS ENABLED: publishing rt/arm_sdk."
        )
        LOG.info(
            "Interface=%s | maximum ownership weight=%.2f | "
            "joint target speed<=%.2f rad/s | IK jump<=%.2f rad",
        ...
        while not FORCE_STOP.is_set():
            ...
            if SNAPSHOT_REQUESTED:
                ...
                if lowstate is not None:
                    ...
                    target_error_text = (
                        f"{np.max(np.abs(arm_ctrl.get_target_q() - measured_q)):.3f} rad"
                    )
                ...
                LOG.info(
                    ...
                    "max_target_error=%s current_q=%s",
                    state.name,
                    arm_ctrl.get_weight(),
