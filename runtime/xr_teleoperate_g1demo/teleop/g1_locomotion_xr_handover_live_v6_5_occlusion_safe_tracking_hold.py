#!/usr/bin/env python3
"""Live G1 Regular-mode locomotion <-> XR arm handover (V6.5 occlusion-safe hold).
...
def update_guarded_pair(
    ...
    V6.5 accepts or holds BOTH wrists together. If either wrist jumps relative
    ...
    # V6.5 safety rule: never accept a discontinuous pair solely because it
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Synchronized V6.4 diagnostic CSV path. Default: "
            "~/g1_setup_logs/xr_v6_4_diag_<timestamp>.csv"
        ),
    )
    return parser
...
def main() -> int:
    ...
    try:
        ...
        LOG.info("G1_LOCOMOTION_XR_HANDOVER_LIVE_V6_5_OCCLUSION_SAFE_TRACKING_HOLD")
        LOG.warning(
        ...
        LOG.info(
            "V6.4 coherent wrist guard: a jump on either wrist holds BOTH "
            "wrist targets until one stable pair is reacquired."
        )
        LOG.info(
        ...
        diagnostic_path = (
            ...
            / "g1_setup_logs"
            / f"xr_v6_4_diag_{time.strftime('%Y%m%d_%H%M%S')}.csv"
        )
