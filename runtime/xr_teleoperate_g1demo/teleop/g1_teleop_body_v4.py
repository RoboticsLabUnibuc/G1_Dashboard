...
# PAUSE_RECENTER_V1
# WRIST_BODY_FRAME_V4_BENT_ELBOW
...
def build_operator_body_frame(
    ...
    """
    Estimate an operator body-yaw frame from the default bent-elbow two-hand pose.
    ...
    The wrist line supplies the lateral axis. Headset yaw is used only during
    calibration to choose the forward-facing branch. The wrist midpoint is not
    required to be in front of the headset because, with elbows bent roughly
    90 degrees, the headset can physically sit ahead of the wrists. Head yaw is
    not used for continuous wrist control after calibration.
    """
    ...
    # With the default roughly-90-degree elbow bend, the Quest headset is
    # often physically in front of the wrists. Therefore a small negative
    # headset-relative reach is normal and must not force straight arms.
    # Keep only a generous sanity bound to reject clearly implausible poses.
    if not np.isfinite(forward_reach):
        raise ValueError("headset-relative wrist reach is non-finite")

    if (
        forward_reach
        < _OPERATOR_FRAME_MIN_HEADSET_RELATIVE_REACH_M
    ):
        raise ValueError(
...
TRANSLATION_PROFILE_VERSION = 4
TRANSLATION_PROFILE_OPERATOR_FRAME = "wrist_body_v4_bent_elbow"
...
if __name__ == '__main__':
    ...
    try:
        ...
        # GUARDED_TRACKING_V1 + WRIST_BODY_FRAME_V4 state.
