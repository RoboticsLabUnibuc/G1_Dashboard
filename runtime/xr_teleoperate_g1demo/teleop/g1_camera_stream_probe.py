...
from vuer.schemas import Hands, MotionControllers, OrbitControls
...
async def patched_main_pass_through(self: TeleVuer, session: Any) -> None:
    ...
    # explicitly request main-camera updates from the browser.
    session.upsert(
        OrbitControls(stream=True, key="xr-camera-controls"),
        to="bgChildren",
    )
    print("[Probe] OrbitControls(stream=True) inserted for explicit camera streaming.", flush=True)
...
def main() -> int:
    ...
    LOG.info("[Probe] CAMERA_STREAM_PROBE_V1 -- read-only")
