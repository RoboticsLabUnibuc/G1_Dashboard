#!/usr/bin/env python3
"""Quest hand-tracking -> Inspire DFX finger-only test V2 for Unitree G1 PC2.
...
def main() -> int:
    ...
    try:
        ...
        # Prevent cleanup helpers from re-entering our normal safety handler.
        try:
        ...
        if tv_wrapper is not None:
            ...
            if close_thread.is_alive():
                LOG.warning(
                    "TeleVuer cleanup exceeded 2.0 s; "
                    "continuing with process termination."
                )
            elif close_error:
                LOG.warning(
                    "TeleVuer close failed: %s",
                    close_error[0],
                )
