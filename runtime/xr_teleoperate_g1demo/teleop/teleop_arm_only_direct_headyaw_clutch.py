...
c : clutch: pause; press again to request aligned resume
...
def main() -> int:
    ...
    try:
        ...
        LOG.info("DIRECT_HEAD_YAW_ARM_ONLY_CLUTCH_V2")
        LOG.info(
        ...
        LOG.info(
            "The clutch does not recenter or calculate a neutral offset. "
            "It preserves direct live head_yaw control."
        )
            ...
            if CLUTCH_TOGGLE:
                ...
                if state == "RUNNING":
                    ...
                    LOG.info(
                        ...
                        "current arm pose and hold still."
                    )
            ...
            if state == "ALIGNING":
                if not frame_valid or live_left is None or live_right is None:
                    ...
                    left_align_error = float(
                        np.linalg.norm(
                            live_left[:3, 3] - current_left[:3, 3]
                        )
                    )
                    right_align_error = float(
                        np.linalg.norm(
                            live_right[:3, 3] - current_right[:3, 3]
                        )
                    )
                    max_align_error = max(
                        left_align_error,
                    ...
                    now = time.monotonic()
                    if max_align_error <= args.start_align_m:
                    ...
                    if now - last_align_log >= 1.0:
                        LOG.info(
                            "Initial alignment: L=%.3f m R=%.3f m "
                    ...
                    if stable_align_count >= args.stable_start_frames:
                        # Direct mode: no neutral/recenter offset is computed.
                        ...
                        # Start the Cartesian limiter at the robot's current FK.
                        ...
                        LOG.info(
                            "ENGAGED: direct live head_yaw targets are now "
                            "driving IK."
                        )
                        LOG.info(
                            "No frozen yaw, relative neutral, profile, body "
                            "frame, axis registration, or operator-yaw offset "
                            "is active."
                        )
            ...
            if state == "RUNNING":
                ...
                if state == "RESUME_ALIGN":
                    if not frame_valid or live_left is None or live_right is None:
                        ...
                        if now - last_align_log >= 1.0:
                            LOG.info(
                                "Resume alignment: L=%.3f m R=%.3f m "
                                "(limit %.3f m), stable=%d/%d.",
                                left_align_error,
                                right_align_error,
                                args.start_align_m,
                        ...
                        if stable_align_count >= args.stable_start_frames:
                            # Re-enter direct absolute control without
                            # calculating any recenter/neutral transform.
                            last_good_left = live_left.copy()
                            last_good_right = live_right.copy()
                            guarded_left = live_left.copy()
                            guarded_right = live_right.copy()
                            ...
                            # Start from the current robot pose so resuming is
                            # smooth even with a small accepted mismatch.
                            last_left_command = current_left.copy()
                            last_right_command = current_right.copy()
                            last_command_time = time.monotonic()
                            consecutive_invalid_frames = 0
                            ...
                            LOG.info(
                                "RESUMED: direct live head_yaw control active. "
                                "No recenter offset was applied."
                            )
