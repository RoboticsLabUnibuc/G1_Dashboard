...
class FingerController:
    def __init__(
        ...
        *,
        ...
        self._status: dict[str, object] = {
            ...
            "tracking_reason": "waiting for Quest",
            "stable_count": 0,
            "fault_count": 0,
        ...
        try:
            ...
            # Establish an open command before loading the retargeter.
            for _ in range(10):
            ...
            try:
                # hand_retargeting.py resolves assets relative to teleop/.
                os.chdir(CURRENT_DIR)
                self._retargeting = HandRetargeting(
    ...
    def close(self) -> None:
        ...
        if self._thread.is_alive():
            LOG.error("Finger worker did not stop within 1.5 seconds.")
    ...
    def _run(self) -> None:
        ...
        try:
            while not self._stop.is_set():
                ...
                tracking_reason = (
                    ...
                    if tracking_valid
                        ...
                        f"motion_ready={motion_ready}; age={tracking_age:.3f}s; "
                ...
                if sequence != last_sequence:
                    ...
                    if tracking_valid:
                        ...
                        stable_count = 0
                        fault_count += 1
                elif tracking_age > self._tracking_stale_s:
                    stable_count = 0
                    fault_count += 1

                if feedback_fault:
                    # Re-publish the last command, but do not change it.
                    ...
                    if (
                        ...
                        reopen_after_feedback = False
                        LOG.info(
                            "Recovery opening complete. Finger state-machine "
                            "control restored."
                        )
                elif mode == FingerMode.OPEN:
                    target_left = np.ones(6, dtype=float)
                    target_right = np.ones(6, dtype=float)
                elif mode == FingerMode.HOLD:
                    target_left = current_left.copy()
                    target_right = current_right.copy()
                elif tracking_valid and (
                    stable_count >= self._stable_tracking_frames
                ):
                    assert left_hand is not None
                    assert right_hand is not None

                    ref_left = (
                        left_hand[self._retargeting.left_indices[1, :]]
                        - left_hand[self._retargeting.left_indices[0, :]]
                    )
                    ref_right = (
                        right_hand[self._retargeting.right_indices[1, :]]
                        - right_hand[self._retargeting.right_indices[0, :]]
                    )

                    raw_left = self._retargeting.left_retargeting.retarget(
                        ref_left
                    )[self._retargeting.left_dex_retargeting_to_hardware]
                    raw_right = self._retargeting.right_retargeting.retarget(
                        ref_right
                    )[self._retargeting.right_dex_retargeting_to_hardware]

                    last_raw_left = np.asarray(raw_left, dtype=float).copy()
                    last_raw_right = np.asarray(raw_right, dtype=float).copy()
                    last_normalized_left = normalize_inspire_target(raw_left)
                    last_normalized_right = normalize_inspire_target(raw_right)

                    target_left = np.maximum(
                        last_normalized_left,
                        self._minimum_command,
                    )
                    target_right = np.maximum(
                        last_normalized_right,
                        self._minimum_command,
                    )

                    # Required hardware workaround, before rate limiting.
                    target_left[5] = 1.0

                    if tracking_fault_latched:
                        LOG.info("Finger tracking recovered; following resumed.")
                        tracking_fault_latched = False
                elif fault_count < self._tracking_fault_frames:
                    # Transient tracking loss: hold the last actual command.
                    target_left = current_left.copy()
                    target_right = current_right.copy()
                else:
                    # Persistent loss: smooth opening instead of indefinite hold.
                    target_left = np.ones(6, dtype=float)
                    target_right = np.ones(6, dtype=float)
                    if not tracking_fault_latched:
                        LOG.error(
                ...
                self._update_status(
                    ...
                    state=(
                        ...
                        else np.asarray(state_values, dtype=float).copy()
                    ),
            ...
            self._update_status(worker_error=self._worker_error)
...
def process_label(pid: int) -> str:
    try:
        ...
        label = data.replace(b"\0", b" ").decode(errors="replace").strip()
...
def kill_all_descendants() -> None:
    ...
    for attempt in range(1, 4):
        ...
        LOG.warning(
            ...
            len(descendants),
            ", ".join(
                f"{pid}:{process_label(pid)}" for pid in descendants
            ),
        )
...
def main() -> int:
    ...
    try:
        ...
        LOG.info("G1_LOCOMOTION_XR_HANDOVER_LIVE_V5_1_ARMS_FINGERS_TELEVUER_STARTUP_FIX")
        LOG.warning(
        ...
        # TeleVuer owns a child/server runtime. Construct it before starting
        # the 60 Hz finger worker thread. Forking/spawning the TeleVuer runtime
        # after another Python worker is active can leave aiohttp/SSL transport
        # state inconsistent and prevent the Quest WebSocket from completing.
        tv_wrapper = TeleVuerWrapper(
        ...
        LOG.info("TeleVuer server initialized before finger worker startup.")
