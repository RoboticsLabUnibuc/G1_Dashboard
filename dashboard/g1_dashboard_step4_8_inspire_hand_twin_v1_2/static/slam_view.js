...
class SlamViewer {
  constructor(canvas){
    this.canvas=canvas;
    this.helpElement=
      canvas.parentElement?.querySelector('.slam-help')||null;
    this.visible=false;
  ...
  bind(){
    ...
    const move=event=>{
      ...
      // Mouse release can occasionally be lost while localization changes
      // the canvas state. Treat buttons=0 as an implicit release.
      if(event.pointerType==='mouse'&&event.buttons===0){
        this.endActiveDrag(event);
        return;
      }
    ...
    const finish=event=>this.endActiveDrag(event);

    this.canvas.addEventListener('pointerdown',event=>{
      if(!this.visible)return;
      if(this.drag)this.endActiveDrag(null);
      if(
        event.button!==0
        && event.button!==1
        && event.button!==2
      ...
      try{
        ...
        // Window-level listeners remain the fallback.
      }
    ...
    // Additional fallbacks for browsers that lose pointerup during a
    // localization/UI transition.
    ...
    document.addEventListener('visibilitychange',()=>{
      if(document.hidden)this.endActiveDrag(null);
    });
  ...
  beginInitialPoseSelection(callback){
    ...
    if(this.helpElement){
      this.helpElement.textContent=
        'LEFT DRAG SET POSE · SHIFT+LEFT / MIDDLE PAN · '
        +'RIGHT / ALT+LEFT ORBIT · WHEEL ZOOM';
    }
  }
  ...
  finishInitialPoseSelection(keepMarker=true){
    ...
    this.initialPoseCallback=null;
    this.endActiveDrag(null);
    this.canvas.classList.remove('selecting-pose');
  ...
  clearInitialPoseSelection(){
    const wasSelecting=this.initialPoseSelecting;
    this.initialPoseDraft=null;

    // LOCALIZED status is polled continuously. Do not cancel an unrelated
    // pan/orbit drag every time that status is rendered.
    if(wasSelecting){
